...
class LaneDataset(Dataset):
    ...
    def __init__(self, dataset_base_dir, json_file_path, args, data_aug=False, save_std=False, seg_bev=False):
        ...
        mean = args.vgg_mean if args.vgg_mean is not None else [0.485, 0.456, 0.406]
        std = args.vgg_std if args.vgg_std is not None else [0.229, 0.224, 0.225]
        ...
        ''' 
        about getPerspectiveTransform, see: https://www.cnblogs.com/jiangnanyanyuchen/p/14244281.html?ivk_sa=1024320u
        about ipm, see: https://blog.csdn.net/u013019296/article/details/122335480, https://zhuanlan.zhihu.com/p/507701366
        about homography, see: https://blog.csdn.net/u011776903/article/details/75109443
        '''
        ...
        if self.use_default_anchor:
            ...
            for i in range(fmap_height):
                for j in range(fmap_width):
                    if fmap_mapping_left_10[i, j] >= 0.5 and fmap_mapping_left_10[i, j] < fmap_width - 0.5:
                        ...
                        fmap_mapping_left_10_interp_weight[i, j, :] = np.array(
                            [up_bound - fmap_mapping_left_10[i, j], fmap_mapping_left_10[i, j] - low_bound])
                    if fmap_mapping_right_10[i, j] >= 0.5 and fmap_mapping_right_10[i, j] < fmap_width - 0.5:
                        ...
                        fmap_mapping_right_10_interp_weight[i, j, :] = np.array(
                            [up_bound - fmap_mapping_right_10[i, j], fmap_mapping_right_10[i, j] - low_bound])
                    if fmap_mapping_left_20[i, j] >= 0.5 and fmap_mapping_left_20[i, j] < fmap_width - 0.5:
                        ...
                        fmap_mapping_left_20_interp_weight[i, j, :] = np.array(
                            [up_bound - fmap_mapping_left_20[i, j], fmap_mapping_left_20[i, j] - low_bound])
                    if fmap_mapping_right_20[i, j] >= 0.5 and fmap_mapping_right_20[i, j] < fmap_width - 0.5:
                        ...
                        fmap_mapping_right_20_interp_weight[i, j, :] = np.array(
                            [up_bound - fmap_mapping_right_20[i, j], fmap_mapping_right_20[i, j] - low_bound])
                    if fmap_mapping_left_40[i, j] >= 0.5 and fmap_mapping_left_40[i, j] < fmap_width - 0.5:
                        ...
                        fmap_mapping_left_40_interp_weight[i, j, :] = np.array(
                            [up_bound - fmap_mapping_left_40[i, j], fmap_mapping_left_40[i, j] - low_bound])
                    if fmap_mapping_right_40[i, j] >= 0.5 and fmap_mapping_right_40[i, j] < fmap_width - 0.5:
                        ...
                        fmap_mapping_right_40_interp_weight[i, j, :] = np.array(
                            [up_bound - fmap_mapping_right_40[i, j], fmap_mapping_right_40[i, j] - low_bound])
            args.fmap_mapping_interp_index = np.concatenate(
                (fmap_mapping_left_10_interp_index, fmap_mapping_right_10_interp_index,
            ...
            args.fmap_mapping_interp_weight = np.concatenate(
                (fmap_mapping_left_10_interp_weight, fmap_mapping_right_10_interp_weight,
    ...
    def preprocess_data_from_json_openlane(self, idx_json_file):
        ...
        if self.new_match:
            frame_x_off_values, frame_z_values, frame_visibility_vectors = np.array(frame_x_off_values), np.array(
                frame_z_values), np.array(frame_visibility_vectors)
            if frame_visibility_vectors.shape[0] > 0:  # frame has lane
                frame_visibility_vectors_expand = np.repeat(np.expand_dims(frame_visibility_vectors, axis=1),
                                                            self.anchor_num, axis=1)
                frame_x_off_norm = np.linalg.norm(np.multiply(frame_x_off_values, frame_visibility_vectors_expand),
                                                  axis=2) / np.sum(frame_visibility_vectors_expand, axis=2)
                for an_id in range(frame_x_off_values.shape[1]):
                    ...
                    if anchor_gts_x_off[matched_gt_id] < self.match_dist_thre_3d:
                        gt_anchors.append(
                            np.vstack([frame_x_off_values[matched_gt_id, an_id], frame_z_values[matched_gt_id]]).T)
        ...
        for i, lane in enumerate(gt_lanes):
            ...
            if self.no_3d:
                ...
                H_g2im, P_g2im, H_crop, H_im2ipm = self.transform_mats_impl(cam_extrinsics, \
                                                                            cam_intrinsics, _label_cam_pitch,
                                                                            _label_cam_height)
                M = H_crop
    ...
    def draw_on_ipm_seg_bev(self, im_ipm, lane_anchor, draw_type='laneline', color=np.asscalar(np.array([1])), width=1):
        for j in range(lane_anchor.shape[0]):
            ...
            if draw_type == 'laneline' and \
                ...
                if not self.use_default_anchor:
                    anchor_x_ipm, _ = homographic_transformation(self.H_g2ipm, self.anchor_grid_x[j],
                                                                 self.anchor_y_steps)
