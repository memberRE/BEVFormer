...
class SpatialCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        bs, num_query, _ = query.size() # TODO: need to check

        D = reference_points_cam.size(3)    # numC, B,HW,z,2(xy)
        indexes = []
        for i, mask_per_img in enumerate(bev_mask): # numC, B,HW,z
            index_query_per_img = mask_per_img[0].sum(-1).nonzero().squeeze(-1) #把每个HW的z值加起来，非零元素个数
            indexes.append(index_query_per_img)
        max_len = max([len(each) for each in indexes])  # max nonzero
